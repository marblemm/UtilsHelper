    ...
    public class ToolStripRendererEx : ToolStripRenderer
    {
        ...
        protected override void OnRenderImageMargin(ToolStripRenderEventArgs e)
        {
            if (e.ToolStrip is ToolStripDropDownMenu)
            {
                ...
                rect.Height -= 2;
                g.SmoothingMode = SmoothingMode.AntiAlias;
                using (LinearGradientBrush brush = new LinearGradientBrush(rect, ColorTable.BackColorHover, Color.White, 90f))
                {
                    Blend blend = new Blend();
                    blend.Positions = new[] { 0f, .2f, 1f };
                    blend.Factors = new[] { 0f, 0.1f, .9f };
                    brush.Blend = blend;
                    rect.Y += 1;
                    rect.Height -= 2;
        ...
        internal void RenderBackgroundInternal(Graphics g, Rectangle rect, Color baseColor, Color borderColor, Color innerBorderColor,
            RoundStyle style, bool drawBorder, bool drawGlass, LinearGradientMode mode)
        {
        ...
        internal void RenderSeparatorLine(Graphics g, Rectangle rect, Color baseColor, Color backColor, Color shadowColor, bool vertical)
        {
            ...
            {
                Blend blend = new Blend();
                blend.Positions = new[] { 0f, .3f, .5f, .7f, 1f };
                blend.Factors = new[] { 1f, .3f, 0f, .3f, 1f };
                brush.Blend = blend;
                using (Pen pen = new Pen(brush))
                {
                    ...
                    brush.LinearColors = new[] { shadowColor, backColor };
                    pen.Brush = brush;
                    if (vertical)
                    {
